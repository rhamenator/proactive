...
- The `time_zone` column in exports is hardcoded to `'America/Detroit'` as a static metadata label. This is explicitly required by the v1 spec ("Time zone label: America/Detroit"). It is not the canvasser's device timezone and is not derived at runtime. Canvassers operating in other timezones should be informed that exported timestamps are in UTC and the label is organizational metadata, not a per-visit timezone.
